...
class SignInFragment : Fragment() {
    ...
    private val viewModel: InicioSesionViewModel by viewModels { SignInViewModelFactory() }
                ...
                JetsurveyTheme {
                    SignIn(
                        onNavigationEvent = { event ->
                            when (event) {
                                is EventoInicioSesion.InicioSesion -> {
                                    viewModel.signIn(event.email, event.password)
                                }
                                EventoInicioSesion.SignUp -> {
                                    viewModel.signUp()
                                }
                                EventoInicioSesion.SignInAsGuest -> {
                                    viewModel.signInAsGuest()
                                }
                                EventoInicioSesion.NavigateBack -> {
