...
sealed class PosibleRespuesta {
    data class SingleChoice(val optionsStringRes: List<Int>) : PosibleRespuesta()
    data class SingleChoiceIcon(val optionsStringIconRes: List<Pair<Int, Int>>) : PosibleRespuesta()
    data class MultipleChoice(val optionsStringRes: List<Int>) : PosibleRespuesta()
    data class MultipleChoiceIcon(val optionsStringIconRes: List<Pair<Int, Int>>) : PosibleRespuesta()
    data class Action(
...
sealed class Answer<T : PosibleRespuesta> {
    object PermissionsDenied : Answer<Nothing>()
    data class SingleChoice(@StringRes val answer: Int) : Answer<PosibleRespuesta.SingleChoice>()
    data class MultipleChoice(val answersStringRes: Set<Int>) :
        Answer<PosibleRespuesta.MultipleChoice>()

    data class Action(val result: SurveyActionResult) : Answer<PosibleRespuesta.Action>()
    data class Slider(val answerValue: Float) : Answer<PosibleRespuesta.Slider>()
}
